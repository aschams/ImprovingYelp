...
def analyze_business(popular_id, data):
    ...
    np.savetxt('scratch/tfidf_features_names_trigrams' + popular_id + '.txt', features, delimiter = ',', fmt = '%s' )
    print(popular_id)
    ...
    param_lin = {'max_depth': 12, 'eta': .3, 'silent': 1, 'objective': 'reg:linear', 'eval_metric' :['rmse', 'mae']}
    param_log = {'max_depth': 12, 'eta': .3, 'silent': 1, 'objective': 'multi:softmax', 'num_class' : 5, 'eval_metric' : ['merror']}
